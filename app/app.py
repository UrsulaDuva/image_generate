...
VIDEO_DEFAULT_MODEL = "bytedance/doubao-seedance-2.0"  # client 内部会归一化为上游官方名
...
async def chat_page(request: Request):
    ...
    return FileResponse(html_path, headers={"Cache-Control": "no-store, must-revalidate"})
...
# Chat 模型信息（展示用，仅作为兜底；真实信息由配置 + 上游 /models 决定）
CHAT_MODEL_INFO = {
    "id": "",
    "display_name": "",
    "owned_by": "",
...
def _fetch_free_models() -> list:
    """从上游 /models 拉取可用模型清单（10 分钟缓存）。
    若环境变量 CHAT_FREE_ONLY=1 则按 -free 后缀过滤；否则全收。"""
    ...
    try:
        ...
        if resp.status_code == 200:
            data = resp.json()
            # 是否仅返回免费模型（id 以 -free 结尾），由环境变量 CHAT_FREE_ONLY 控制
            free_only = (os.environ.get('CHAT_FREE_ONLY', '') or '').strip() in ('1', 'true', 'yes')
...
async def chat_completion(request: Request, body: ChatRequest):
    """文案生成 - OpenAI 兼容 chat completion（支持流式）"""
    username = verify_session(request)
    if not username:
        raise HTTPException(status_code=401, detail="未登录")
    ...
    if not any(m.get("role") == "system" for m in msgs):
        ...
        msgs.insert(0, {
            "role": "system",
            "content": f"你是「AI 图像工坊」的文案助手。当前时间：{now_str}。如用户提到“今天/最新/近期”等时间相关内容，请基于此时间回答。"
        })

    # 兼容前端浏览器缓存的旧模型 id：若 body.model 在白名单（_fetch_free_models）外则强制用后端配置
    requested_model = body.model or model
    try:
        free_ids = {m["id"] for m in _fetch_free_models()}
    except Exception:
        free_ids = set()
    if free_ids and requested_model not in free_ids:
        print(f"[chat] override stale model '{requested_model}' -> '{model}'")
    ...
    async def stream_gen():
        import asyncio
        try:
            # aiohttp 3.13 要求 ssl 参数是 SSLContext/bool/Fingerprint/None
            # 不能直接传 PEM 路径字符串（会抛 TypeError 把整个流挂掉，
            # 然后前端只看到“文案生成失败，请稍后重试”，根因被吞）。
            ssl_arg: object = True
            for p in ('/etc/ssl/cert.pem', '/opt/homebrew/lib/python3.13/site-packages/pip/_vendor/certifi/cacert.pem'):
                if os.path.exists(p):
                    try:
                        import ssl as _ssl
                        ssl_arg = _ssl.create_default_context(cafile=p)
                    except Exception:
                        ssl_arg = True
                    break
            # 使用 aiohttp 进行异步流式请求
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(api_url, headers=headers, json=payload, timeout=180, ssl=ssl_arg) as resp:
                    if resp.status != 200:
                        text = await resp.text()
                        print(f"[chat] upstream HTTP {resp.status}: {text[:300]}")
                        yield f"data: {{\"error\": \"文案生成服务暂时不可用\"}}\n\n"
                        return
            ...
            yield f"data: {{\"error\": \"文案生成失败，请稍后重试\"}}\n\n"
