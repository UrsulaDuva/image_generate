#!/usr/bin/env python3
"""
视频生成客户端 - 兼容 Vertex AI 协议的网关封装

支持模型: bytedance/doubao-seedance-2.0（默认）

调用流程: submit -> poll(15s) -> download

通过环境变量 VIDEO_BASE_URL 指定上游 Vertex 网关地址，
未设置时回退到 https://your-vertex-gateway.example.com/api/vertex-ai
"""
...
VERTEX_BASE_URL = os.environ.get(
    "VIDEO_BASE_URL", "https://your-vertex-gateway.example.com/api/vertex-ai"
)
...
def _normalize_model(model: str) -> str:
    """部分上游仅接受 bytedance/doubao-seedance-2.0；统一别名"""
    if not model:
        return model
    m = model.strip()
    aliases = {
...
def _build_client(api_key: str):
    """构造 google-genai 客户端（指向上游 Vertex 网关）"""
    try:
